...
void ModelImporter::GLTF_To_AST(const char* filePathRel , uint8_t scalingFactorBits, uint8_t FPS, uint8_t tPageX_begin, uint8_t tPageY_begin, uint8_t tPageX_end, uint8_t tPageY_end) {
	...
	tempByte |= (uint8_t)!ImpMeshes.empty() << 7;
	if (!ImpMeshes.empty()) {
		tempByte |= (uint8_t)!ImpMeshes[0].triangleVerts.empty() << 6;
		tempByte |= (uint8_t)!ImpMeshes[0].indices.empty() << 5;
		tempByte |= (uint8_t)!ImpMeshes[0].textureCoords.empty() << 4;
		tempByte |= (uint8_t)!ImpMeshes[0].normals.empty() << 3;
	}
	tempByte |= (uint8_t)!bones.empty() << 2;
	if (!bones.empty()) {
		tempByte |= (uint8_t)!bones[0].animations.empty()<<1;
	}

	astStream.write(reinterpret_cast<char*>(&tempByte), sizeof(tempByte));

	astStream.write(reinterpret_cast<char*>(&scalingFactorBits), sizeof(scalingFactorBits));

	astStream.write(reinterpret_cast<char*>(&tPageX_begin), sizeof(tPageX_begin));
	astStream.write(reinterpret_cast<char*>(&tPageY_begin), sizeof(tPageY_begin));
	...
	for (MImesh msh : ImpMeshes)
	{
		...
		astStream.write(reinterpret_cast<char*>(&tempShort),sizeof(tempShort));
		...
		for (uint16_t i = 0; i < tempShort; i+=3)
		{
			...
			astStream.write(reinterpret_cast<char*>(&tempFix), sizeof(tempFix));
		...
		for (uint16_t i = 0; i < tempShort; i += 2)
		{
			...
			tempByte = (uint8_t)((uint16_t)round((msh.textureCoords[i] / ((float)x_diff / (float)(x_diff - 1))) * (float)(x_diff)) % x_diff);

			astStream.write(reinterpret_cast<char*>(&tempByte), sizeof(tempByte));

			tempByte = (uint8_t)((uint16_t)round((msh.textureCoords[i + 1] / ((float)y_diff / (float)(y_diff - 1))) * (float)(y_diff)) % y_diff);
